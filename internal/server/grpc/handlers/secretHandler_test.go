...
func TestSecretHandler_SaveUserSecret(t *testing.T) {
	...
	logger := zap.NewNop()
	handler := NewSecretHandler(logger, mockService)
	...
	tests := []struct {
	...
	}{
		{
			name: "Success_Create",
			setupMock: func() {
				mockService.EXPECT().CreateSecret(gomock.Any(), gomock.Any()).Return(&models.Secret{ID: 1}, nil).Times(1)
			},
			ctx: metadata.NewIncomingContext(
				context.WithValue(context.Background(), consts.CtxUserIDKey, uint64(123)),
		...
		},
		{
			...
			setupMock: func() {
				mockService.EXPECT().CreateSecret(gomock.Any(), gomock.Any()).Return(nil, errors.New("create error")).Times(1)
			},
			ctx: metadata.NewIncomingContext(
				context.WithValue(context.Background(), consts.CtxUserIDKey, uint64(123)),
				metadata.New(map[string]string{consts.ClientIDHeader: "456"}),
...
func TestSecretHandler_GetUserSecret(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "Success",
			setupMock: func() {
				mockService.EXPECT().GetSecret(gomock.Any(), uint64(1), uint64(123)).Return(&models.Secret{ID: 1}, nil).Times(1)
			},
			ctx: context.WithValue(context.Background(), consts.CtxUserIDKey, uint64(123)),
			input: &proto.GetUserSecretRequest{
				Id: 1,
			},
			expectErr: "",
		},
		{
			name: "Error_NotFound",
			setupMock: func() {
				mockService.EXPECT().GetSecret(gomock.Any(), uint64(1), uint64(123)).Return(nil, errors.New("secret not found")).Times(1)
			},
			ctx: context.WithValue(context.Background(), consts.CtxUserIDKey, uint64(123)),
			input: &proto.GetUserSecretRequest{
				Id: 1,
...
func TestSecretHandler_GetUserSecrets(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	tests := []struct {
	...
	}{
		{
			name: "Success",
			setupMock: func() {
				mockService.EXPECT().GetUserSecrets(gomock.Any(), uint64(123)).Return(models.Secrets{}, nil).Times(1)
			},
			ctx: metadata.NewIncomingContext(
				context.WithValue(context.Background(), consts.CtxUserIDKey, uint64(123)),
				metadata.New(nil),
...
func TestSecretHandler_DeleteUserSecret(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "Success",
			setupMock: func() {
				mockService.EXPECT().DeleteSecret(gomock.Any(), uint64(1), uint64(123)).Return(nil).Times(1)
			},
			ctx: metadata.NewIncomingContext(
				context.WithValue(context.Background(), consts.CtxUserIDKey, uint64(123)),
				metadata.New(nil),
		...
		},
		{
			name: "Error_NotFound",
			setupMock: func() {
				mockService.EXPECT().DeleteSecret(gomock.Any(), uint64(1), uint64(123)).Return(fmt.Errorf("secret not found (id=%d)", 1)).Times(1)
			},
			ctx: metadata.NewIncomingContext(
				context.WithValue(context.Background(), consts.CtxUserIDKey, uint64(123)),
				metadata.New(nil),
		...
		},
		{
			name: "Error_Internal",
			setupMock: func() {
				mockService.EXPECT().DeleteSecret(gomock.Any(), uint64(1), uint64(123)).Return(errors.New("internal error")).Times(1)
			},
			ctx: metadata.NewIncomingContext(
				context.WithValue(context.Background(), consts.CtxUserIDKey, uint64(123)),
				metadata.New(nil),
